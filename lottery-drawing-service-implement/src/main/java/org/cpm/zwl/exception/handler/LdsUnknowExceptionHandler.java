...
@ControllerAdvice
public class LdsUnknowExceptionHandler {
	
	private final Logger logger = ZwlLogFactory.getLogger(LdsUnknowExceptionHandler.class);
    ...
    public JsonResponse<String> handlerError(HttpServletRequest req, Exception e) {
		...
		JsonResponse<String> response = JsonResponseUtil.getFail(ZwlBaseErrorCode.UNKNOWN_ERROR.getErrorCode(), e.getMessage());
        return response;
    }
